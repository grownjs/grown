...
module.exports = ($, opts, plugs) => {
  debug('Loading middleware: serve-static');

  $.mount(require('serve-static')(opts.publicDir));

  debug('Loading plugins: models, render, router from');

  $.use(plugs.models(opts.appDir, __dirname));
  $.use(plugs.render(opts.appDir, __dirname));
  ...
  if (opts.uploads !== false) {
    ...
    $.use(plugs.upload(opts.uploads || { maxFields: 10, multiples: true }));
  }
};
