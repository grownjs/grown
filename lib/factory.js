...
function _fixedName(Factory, _name) {
  if (!Factory.name) {
    return _name;
  }

  if (Factory.name === 'anonymous') {
    return _name;
  }
...
module.exports = (Factory, options, _name) => {
  ...
  if (Factory.prototype && typeof Factory.prototype.call === 'function') {
    return {
      name: _fixedName(Factory, 'anonymous'),
