...
module.exports = args => {
  ...
  return $ => {
    ...
    function _send(tpl, locals, _transport) {
      ...
      return Promise.resolve()
        .then(() => (typeof tpl === 'string' ? _view(tpl, _context) : locals))
        .then(_locals => {
          _transport = _locals.transport || _transport;

          tpl = _locals.layout || tpl;

          delete _locals.transport;
          delete _locals.layout;

          // pass extensions as static context
          return _render({ src: tpl, data: _locals }, $.util, _context)
            .then(result => new Promise((resolve, reject) =>
              _proxy(locals.transport || _transport, _context)
              .sendMail({
                subject: _locals.subject || locals.subject,
                from: _locals.from || locals.from,
                to: _locals.to || locals.to,
                html: result,
              }, (err, info) => {
                if (err) {
                  reject({
    ...
    $.on('repl', repl => {
      repl.defineCommand('mailto', {
        ...
        action(value) {
          ...
          // FIXME:
          //
          //  .mail [TO] [SUBJECT] [BODY] [local=value] [foo=bar] [...]
          //    --preview|-p
          //    --mailer|-m
          //    --send|-s -cco|-o --cc|-c
          //    --to|-t --subject|-u --body|-b --attach|-a
          //
          //  .mail [default] --preview
          //  .mail pateketrueke@gmail.com "Hello" "WORLD"
          //  .mail user.activate email=foo@candy.bar code=123 --send
          //  .mail "my subject" `JSON.stringify(process.env, null, 2)` --mailer external
          //
