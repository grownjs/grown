      ...
      try {
        ...
        if (options.headers['content-type']) {
          ...
          if (_body && typeof _body === 'object' && options.headers['content-type'] === 'multipart/form-data') {
            ...
            _body = Object.keys(_body).map(key => {
              if (typeof _body[key] === 'string' && _body[key].charAt() === '@') {
                const filename = path.basename(_body[key].substr(1));

                return Buffer.concat([
                  new Buffer(`--${_boundary}\r\nContent-Disposition: form-data; name="${key}"; filename="${filename}"\r\n`, 'ascii'),
                  new Buffer(`Content-Type: ${mime.lookup(_body[key].substr(1))}\r\n\r\n`, 'ascii'),
                  new Buffer(fs.readFileSync(_body[key].substr(1)), 'ascii'),
                ]);
              }

              return new Buffer(`--${_boundary}\r\nContent-Disposition: form-data; name="${key}"\r\n\r\n${
                typeof _body[key] === 'object' ? JSON.stringify(_body[key]) : _body[key]
              }`, 'ascii');
            }).join('\r\n');
