...
const debug = require('debug')('homegrown:models');
...
module.exports = function $models() {
  ...
  _slice.call(arguments).forEach((cwd) => {
    ...
    debug('Loading database config from %s', _database);
    ...
    _tasks.push(() => {
      ...
      debug('Listing all models from %s', cwd);
    ...
    $.up(() => Promise.all(_tasks.map(cb => cb()))
      .then((results) => {
        ...
        $.extensions('Homegrown.conn', {
          init() {
            this.put_local('models', $.extensions.models);
          },
          props: {
            models: _refs,
          },
