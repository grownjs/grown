    ...
    $.on('listen', () =>
      $.mount('resource', conn => {
        ...
        if (_model && typeof conn.routes === 'function') {
          ...
          [_model.name]
            ...
            .forEach(model => {
              ...
              if (!conn.resources[model]) {
                throw new Error(`${model} resource is missing`);
              }
          ...
          Object.keys(resource.options.refs).forEach(prop => {
            const m = resource.options.refs[prop].model;

            /* istanbul ignore else */
            if (_model.name !== m) {
              resource.options.uiSchema[prop] = util.extend({},
                resource.options.uiSchema[prop],
                _refs[m].$uiSchema || {});
            }
          });
