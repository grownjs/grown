...
module.exports = args => {
  ...
  return $ => {
    ...
    function _fixParams(data, props) {
      Object.keys(props).forEach(key => {
        if (props[key].type === 'BelongsTo' && data[key]) {
    ...
    function _getOpts(model, params) {
      const obj = {
    ...
    $.on('listen', () => {
      $.mount('models', conn => {
        ...
        if (_model && conn.routes) {
            ...
            Object.keys(model.refs).map(key => model.refs[key].target)
              .forEach(m => {
                ...
                if (conn.resources[m.name]) {
                  add(m.name,
                    conn.routes(conn.resources[m.name].controller));
                }
              });
          }
          ...
          push(_model);
          add(_model.name,
            conn.routes(_handler.controller));
          ...
          conn.put_local('resource', util.extend({}, _model.definition, {
            $uiFields: Object.keys(_model.definition.$schema.properties)
              .map(prop => ({ prop })),
            $uiSchema: {},
          }));
          ...
          switch (_handler.action) {
            case 'index':
              return _model.findAll(_getOpts(_model))
              .then(data => {
            ...
            case 'show':
            case 'edit':
              return _model.findOne(_getOpts(_model, { where: _where }))
              .then(data => {
                /* istanbul ignore else */
                if (!data) {
            ...
            case 'update':
              ...
              return Promise.resolve()
                .then(() => {
                  ...
                  const payload = _fixParams(conn.params.payload, _props);
                  ...
                  return Promise.all(Object.keys(_props)
                    ...
                    .then(() => _model.update(payload, _getOpts(_model, { where: _where })));
                })
            ...
            case 'create':
              ...
              return _model.create(_fixParams(conn.params.payload, _props), _getOpts(_model))
                .then(result => ok(conn, result, _actions[_model.name].index.path))
            ...
            case 'destroy':
              ...
              return _model.destroy(_getOpts(_model, { where: _where }))
                .then(result => ok(conn, result, _actions[_model.name].index.path))
