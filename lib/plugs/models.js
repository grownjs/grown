...
module.exports = args => {
  ...
  return $ => {
    ...
    // array-like wrapper
    const _refs = $.extensions('Conn._.models');

    // store connections
    const _dbs = $.extensions('Conn._.dbs');
      ...
      $.mount('resource', conn => {
        ...
        if (_model && typeof conn.routes === 'function') {
          ...
          return JSONSchemaSequelizer
            .resource(conn, _model, _handler.action).then(res => {
              ...
              if (_handler.action === 'create' || _handler.action === 'update' || _handler.action === 'destroy') {
                return ok(conn, res.result);
              }

              /* istanbul ignore else */
              if (!conn.is_xhr) {
                conn.put_local('resource', res.get());
              }

              /* istanbul ignore else */
              if (res.isNew || _handler.action === 'edit') {
                return ok(conn, res.get());
              }

              return ok(conn, res.data);
            })
            .catch(e => err(conn, e));
        }
      }));
    ...
    $.on('start', () => Promise.all(_tasks.map(cb =>
      ...
      .then(results => {
        ...
        // merge all models
        results.forEach(x => {
          util.extend(_refs, x.sequelize.models);
