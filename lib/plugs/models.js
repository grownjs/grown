...
module.exports = args => {
  ...
  ((!Array.isArray(args) && args ? [args] : args) || []).forEach(opts => {
    ...
    _config.operatorsAliases = typeof _config.operatorsAliases !== 'undefined'
      ? _config.operatorsAliases
      : false;
  ...
  return $ => {
    ...
    $.extensions('Conn', {
      props: {
        ...
        models() {
          /* istanbul ignore else */
          if (typeof this.database === 'function') {
            this.database = this.database();
          }

          /* istanbul ignore else */
          if (!this.database || !_dbs[this.database]) {
            throw new Error(`Unknown database ${this.database}`);
          }

          // overload references
          return _dbs[this.database].models;
        },
      },
    });
    ...
    $.on('listen', () =>
      $.mount('resource', conn => {
        ...
        /* istanbul ignore else */
        if (typeof conn.database === 'function') {
          conn.database = conn.database();
        }

        /* istanbul ignore else */
        if (!conn.database || !_dbs[conn.database]) {
          throw new Error(`Unknown database ${conn.database}`);
        }

        const _refs = _dbs[conn.database].refs;
        const _model = buildModel(_dbs[conn.database].models, _handler);
        ...
        const resource = JSONSchemaSequelizer.resource(_refs, _model, {
          [_model.primaryKeyAttribute]: conn.params[_model.primaryKeyAttribute],
        ...
        if (typeof _model.setResource === 'function') {
          _model.setResource(conn, resource);
        }
