...
module.exports = args => {
  ...
  return $ => {
    ...
    const graphql = JSONSchemaSequelizer.graphql(modelName => {
      if (!_models[modelName]) {
        throw new Error(`Missing model definition for ${modelName}`);
      }
    ...
    $.extensions('Conn', {
      ...
      before_send() {
        ...
        Object.keys(_model.options.$schema.properties).forEach(key => {
          ...
          /* istanbul ignore else */
          if (x) {
            const dest = $.options(typeof x === 'string' ? x : 'paths.uploadsDir');
