...
module.exports = args => {
  ...
  return $ => {
    ...
    function buildModel(handler) {
      const definition = _models[handler.resource];
    ...
    $.on('listen', () =>
      $.mount('resource', conn => {
        ...
        const _model = buildModel(_handler);
    ...
    $.on('start', () => Promise.all(_tasks.map(cb =>
      ...
      .then(results => {
        ...
        // merge all models
        results.forEach(x => {
          if (x.sequelize.options.global === true) {
          ...
          // convert namespaces
          const key = util.ucwords(x.sequelize.options.identifier, '', false);
