...
module.exports = args => {
  ...
  return $ => {
    ...
    $.on('listen', () => {
      $.mount('models', conn => {
        ...
        if (_model && conn.routes) {
          ...
          _refs.refs.forEach(ref => {
            /* istanbul ignore else */
            if (!_refs[ref.id]) {
              _fixed[ref.id] = { $schema: ref };
              return;
            }

            _fixed[ref.id] = {
              $schema: ref,
              $uiFields: _refs[ref.id].definition.$uiFields,
              $uiSchema: _refs[ref.id].definition.$uiSchema,
            };
          });
          ...
          switch (_handler.action) {
            case 'index':
              return _model.findAll({
                include: [_model.refs.brand, _model.refs.company],
              })
              .then(data => {
    ...
    $.on('start', () => Promise.all(_tasks.map(cb => cb()))
      .then(results => {
        const _schemas = util.proxyArray();
        const _sync = [];
        ...
        results.forEach(models => {
          ...
          Object.keys(models.refs).forEach(ref => {
            _schemas[ref] = models.refs[ref];
          });
        });

        util.setProperty(_refs, 'refs', _schemas);
