    ...
    $.on('listen', () => {
      $.mount('models', conn => {
        ...
        if (_model && conn.routes) {
          ...
          function add(name, routes, property) {
            _actions[name] = {};
            ...
            .forEach(m => {
              ...
              if (conn.resources[m.name]) {
                add(m.name,
                  ...
                  `${m.options.name.singular.toLowerCase()}_${_model.primaryKeyAttribute}`);
              }
            });

          add(_model.name, _mappings, _model.primaryKeyAttribute);
          ...
          Object.keys(_model.refs)
            .forEach(ref => {
              _props[ref] = _model.refs[ref].associationType;

              /* istanbul ignore else */
              if (!_models[_model.refs[ref].target.name]) {
                _models[_model.refs[ref].target.name] = _model.refs[ref].target.definition.$schema;
              }
            });

          conn.put_local('resource', _model.definition);
          ...
          switch (_handler.action) {
            ...
            case 'update':
              ...
              // FIXME: update each related field...
