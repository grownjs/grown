...
module.exports = args => {
  ...
  return $ => {
    ...
    function _getRefs(model) {
      return Object.keys(model.refs).map(x => model.refs[x]);
    }
    ...
    $.on('listen', () => {
      $.mount('models', conn => {
        ...
        if (_model && conn.routes) {
          ...
          Object.keys(_model.refs).map(key => _model.refs[key].target)
            .forEach(m => {
              /* istanbul ignore else */
          ...
          switch (_handler.action) {
            ...
            case 'edit':
              ...
              .then(data => {
                ...
                conn.put_local('resource.$result', data);
              });
            ...
            case 'update':
              ...
              return Promise.resolve()
                .then(() =>
                  Promise.all(Object.keys(_props)
                    .map(key =>
                      _props[key].type === 'HasOne'
                      &&  _model.refs[key].target.update(payload[key], { where: _where }))))
                .then(() => _model.update(payload, {
                  where: _where,
                  include: _getRefs(_model),
                }))
            ...
            case 'create':
              ...
              return _model.create(_fixParams(conn.params.payload, _props), {
                include: _getRefs(_model),
              })
              .then(result => ok(conn, result, _actions[_model.name].index.path))
