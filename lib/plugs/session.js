...
module.exports = function $session(defaults) {
  ...
  return ($, util) => {
    $.mount('cookie-parser', cookieParser(util.extend({}, defaults)));
    $.mount('cookie-session', cookieSession(util.extend({}, defaults)));

    $.extensions('Homegrown.conn', {
      before_send() {
        this.put_local('csrf_token', this.req.csrfToken && this.req.csrfToken());
      },
      props: {
        session() {
