...
module.exports = args => {
  return $ => {
    ...
    const _defaults = $.extensions('Conn._.access', false);
    ...
    ((!Array.isArray(args) && args ? [args] : args) || []).forEach(opts => {
      /* istanbul ignore else */
      if (typeof opts.settings !== 'string' || !$.util.fs.exists(opts.settings)) {
      ...
      if (_defaults.callback && typeof _defaults.callback !== 'function') {
        throw new Error(`Expecting 'opts.callback' to be a function, given '${opts.callback}' does not exists`);
      }
