  ...
  return $ => {
    ...
    function run(conn, _options) {
      ...
      if (_handler) {
        ...
        _controllers[_handler.controller] = _controllers[_handler.controller] || { pipeline: {} };

        // read from memory first
        const Controller = util.ctx.load(_extensions.controllers,
        ...
        if (!_pipeline) {
          ...
          _pipeline.push({
            ...
            call: _conn => Promise.resolve()
              .then(() => controllerInstance && controllerInstance[_handler.action](_conn))
              .then(retval => {
                /* istanbul ignore else */
                if (retval) {
        ...
        return _pipeline(conn, _options)
          .then(() => {
            ...
            if (!conn.halted && typeof conn.view === 'function') {
              ...
              const _partials = [
                `views/${_handler.controller.replace(reDotSeparator, '/')}/${_handler.action}`,
              ];

              if (conn.handler.resource) {
    ...
    function call(conn, group, options, middleware) {
      if (typeof _scopes[group] === 'undefined') {
        // http-middleware is required (if exists)
        _scopes[group] = _middlewares.http
          ? ['http']
          : [];

        // env-middleware is required (if exists)
        _scopes[group] = _middlewares[$.config('env')]
          ? _scopes[group].concat($.config('env'))
          : _scopes[group];

        if (Array.isArray(middleware)) {
          _scopes[group] = _scopes[group].concat(middleware);
        }

        _scopes[group] = _scopes[group].length
          ? pipelineFactory(group, _require(_scopes[group], options))
          : false;
      }
      ...
      return Promise.resolve()
        .then(() => {
          if (_scopes[group]) {
            return _scopes[group](conn, options);
          }
        });
    }
    ...
    $.extensions('Conn', {
      ...
      methods: {
        use(name, options, middleware) {
          return call(this, name, options, middleware);
        },
      },
      props: {
