...
module.exports = function routerPlugin() {
  ...
  return ($, util) => {
    // public API
    $.ctx.extensions.routes = router.mappings;
    $.ctx.extensions.routes.map = Array.prototype.map.bind(_routes);
    $.ctx.extensions.routes.forEach = Array.prototype.forEach.bind(_routes);

    // controller holder
    $.ctx.extensions.controllers = {};
      ...
      if (_handler) {
        ...
        if (!Controller) {
          ...
          if (typeof Controller.init === 'function' || Controller.methods) {
            Controller = objectNew(_handler.controller, Controller, $.ctx.extensions.controllers);
          } else {
            $.ctx.extensions.controllers[_handler.controller] = Controller;
          }
