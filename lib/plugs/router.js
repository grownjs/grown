    ...
    function run(conn, _options) {
      ...
      if (_handler) {
        ...
        return _pipeline(conn, _options)
          .then(() => {
            /* istanbul ignore else */
            if (!conn.is_finished && conn.view) {
              return conn.view(`${_handler.controller}/${_handler.action}`, conn.resp_locals)
                .then(() => conn.end());
            }
          });
      }
      ...
      $.mount('router', (conn, _options) => {
        ...
        if (typeof _http === 'undefined') {
          _http = _middlewares.http
            ? pipelineFactory('$http', _require(['http'], _options))
            : false;
        }
