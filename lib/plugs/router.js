...
const debug = require('debug')('homegrown:router');
...
module.exports = function $router() {
  ...
  function _require(map, options) {
    ...
    debug('Trying to resolve %s module%s: %s',
      map.length,
      map.length === 1 ? '' : 's',
      map.join(', '));
  ...
  function _pipe(from, handler) {
    ...
    debug('Trying to resolve pipeline from %s.%s', handler.controller, handler.action);
  ...
  args.forEach((cwd) => {
    ...
    if (fs.existsSync(_routeMappings)) {
      ...
      router.namespace('/', routeMappings(new RouteMappings({ cwd })));
    }

    // load global middleware definitions
    const _middlewaresFile = path.join(cwd, 'config', 'middlewares.js');

    debug('Loading middlewares from %s', _middlewaresFile);
  ...
  debug('Done. Trying to optimize route mappings');
  ...
  router.routes.forEach((route) => {
    ...
    // cleanup
    delete route._actionName;
    delete route._resourceName;
  ...
  debug('Done. Building tree for matching by method');
  ...
  return ($, util) => {
    debug('Done. Binding routes and controllers');
    ...
    $.extensions('Homegrown.conn', {
      init() {
        ...
        this.put_local('controllers', $.extensions.controllers);
      },
      props: {
    ...
    function run(conn, _options) {
      ...
      /* istanbul ignore else */
      if (!match[_method]) {
        debug('Wait. There is no routes matching for this verb: %s', _method);
      ...
      if (_handler) {
        debug('Done. Trying to load handler for %s controller', _handler.controller);
        ...
        if (_handler.matcher && _handler.matcher.keys) {
          debug('Setting request parameters');
        ...
        /* istanbul ignore else */
        if (!Controller) {
          debug('Controller %s not loaded. Trying to load controller',
            _handler.controller);
          ...
          debug('Done. Registering %s controller instance', _handler.controller);
        ...
        /* istanbul ignore else */
        if (!_pipeline) {
          debug('Initializing the %s.%s pipeline',
            _handler.controller,
            _handler.action);
          ...
          if (Array.isArray(_handler.route.middleware)) {
            debug('Wait. Push middleware pipeline: ',
          ...
          _pipeline.push({
            ...
            call(_conn) {
              ...
              if (Controller.inject) {
                return util.reduce(Controller.inject, cb => cb(conn, _options))
                  .then((deps) => {
                    objectNew.mergePropertiesInto(_conn, deps);
                  });
              }
            },
            type: 'function',
          ...
          debug('Push handler pipeline: %s.%s', _handler.controller, _handler.action);
          ...
          debug('Done. Trying to compile the %s.%s pipeline',
            _handler.controller,
            _handler.action);
        ...
        debug('Trying to execute the %s.%s pipeline',
          _handler.controller,
          _handler.action);
      ...
      debug('Wait. There is no routes matching for this path: %s', _base);
