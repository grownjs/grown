...
module.exports = args => {
  ...
  router.routes.forEach(route => {
    ...
    const action = route._actionName
      ? (_handler.pop() && route._actionName)
      : 'index';

    if (_handler.length && route._actionName) {
      _handler[_handler.length - 1] = route._resourceName;
    }

    const controller = _handler.join('.') || route._resourceName;
