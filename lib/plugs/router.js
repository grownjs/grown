...
module.exports = function $router() {
  ...
  function loadTask(cwd, bootDir, configDir) {
    const _routeMappings = path.join(cwd, configDir, 'routes.js');
    ...
    const _middlewaresFile = path.join(cwd, configDir, 'middlewares.js');
    ...
    glob.sync(path.join(bootDir, 'middlewares/**/*.js'), { cwd, nodir: true }).forEach(middleware => {
  ...
  function build(appDir) {
    router.routes.forEach(route => {
      ...
      const controllerFile = path.join(route.cwd, appDir, 'controllers', `${controller}.js`);
  ...
  return ($, util) => {
    args.unshift($.get('cwd', process.cwd()));

    args.forEach(cwd =>
      loadTask(cwd, $.get('appDir', 'app'), $.get('configDir', 'config')));

    build($.get('appDir', 'app'));
