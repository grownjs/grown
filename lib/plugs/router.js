...
module.exports = args => {
  ...
  ((!Array.isArray(args) && args ? [args] : args) || []).forEach(opts => {
    ...
    if (opts.middlewares) {
      ...
      ((!Array.isArray(opts.middlewares) && opts.middlewares ? [opts.middlewares] : opts.middlewares) || []).forEach(sub => {
        ...
        debug('Loading settings from %s', opts.middlewares.settings);
        ...
        ((!Array.isArray(sub.folders) && sub.folders ? [sub.folders] : sub.folders) || []).forEach(cwd => {
          glob.sync('**/*.js', { cwd, nodir: true }).forEach(src => {
            ...
            if (typeof _fixedMiddlewares[middlewareName] === 'undefined') {
              debug('Registering new middleware from %s', path.join(cwd, src));
            } else {
              debug('Overriding middleware from %s', path.join(cwd, src));
            }
    ...
    debug('Loading routes from %s', opts.settings);
  ...
  return ($, util) => {
    ...
    function run(conn, _options) {
      ...
      if (_handler) {
        debug('#%s Done. Loading handler for <%s> controller', conn.pid, _handler.controller);
        ...
        if (!_pipeline) {
          ...
          _pipeline.push({
            ...
            call: _conn => {
              const retval = controllerInstance[_handler.action](_conn);

              /* istanbul ignore else */
              if (retval !== null
                && (typeof retval === 'object' || typeof retval === 'string')) {
          ...
          debug('#%s Done. Compiling pipeline <%s.%s>',
            conn.pid,
            _handler.controller,
            _handler.action);
        ...
        return _pipeline(conn, _options)
          .then(() => {
            ...
            if (conn.resp_body === null && conn.render) {
              conn.before_send(() => {
                conn.resp_body = conn.render(`${_handler.controller}/${_handler.action}`);
              });
      ...
      debug('#%s Wait. There are no routes matching for this path', conn.pid);
        ...
        action(value) {
          ...
          $.extensions.routes.forEach(x => {
            ...
            if (!_filter || _lookup.indexOf(_filter) > -1) {
              logger.write(`  ${x.route.verb.toUpperCase()} ${x.route.path} (${x.route.as})\n`);
          ...
          /* istanbul ignore else */
          if (!_found) {
            logger.fail('No routes were found');
          } else {
            logger.ok(`${_found} route${_found === 1 ? '' : 's'} ${_found === 1 ? 'was' : 'were'} found`);
          }
        },
      });
    });
