...
module.exports = opts => {
  ...
  return $ => {
    ...
    // this look-up pattern should be applied on json-schema-sequelizer:
    //   src/Type.js  [all-in-one]
    //   src/Type/resolvers.js*  ->  Model.graphqlResolvers
    //   src/Type/schema.graphql*  ->  Model.$graphql
    // all schemas CAN be placed but will not be loaded, just
    // require them through the index.js module; the same with
    // hooks, methods, etc. only Model.js or Model/index.js is required
    // only models can provide $graphql and graphqlResolvers from their class

    // iterate over specified folders looking for schemas/resolvers
    // once started, collect all public models' schemas and resolvers
    // merge everything before building the middleware
    // virtual models can be passed as resolver(VirtualModel) ??
    // how to enable faked calls for findOne/All calls inside graphql-sequelize ??
    ...
    $.on('start', () => {
      ...
      _schema = makeExecutableSchema({
        ...
        resolvers: $.util.extend.apply(null, resolvers),
      });
