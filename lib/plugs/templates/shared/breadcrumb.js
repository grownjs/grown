...
function map(state, routes, query) {
  return routes.map((kp, i) => {
    ...
    if (!h.url && last) {
      return aLink(state.request_path, state.subtitle || state.handler.as.split('.').pop(), query);
    }

    return h && h.url
      ? aLink(h.url(state.path_params), h.as.split('.').pop(), last ? query : null)
      : null;
  });
}
...
module.exports = state => {
  if (!(state.routes || state.handler)) {
    return '<p>Missing routes and handler, verify your state</p>';
  }
