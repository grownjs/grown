...
module.exports = opts => {
  return $ => {
    ...
    const baseDir = path.join(_cwd, opts.baseDir || 'src');
    const assetsDir = path.join(baseDir, _paths.assets || 'assets');
    const contentDir = path.join(baseDir, _paths.content || 'content');
    const templatesDir = path.join(baseDir, _paths.templates || 'templates');
    ...
    $.mount('tarima', conn => {
      ...
      const src = conn.request_path.replace(/^\//, '') || 'index';

      // for other assets, just render as-is
      if (/\.(js|css)$/.test(src)) {
        return conn.next(() => conn.bundle(`${assetsDir}/${src}`))
          .then(partial => {
      ...
      // skip extensions
      if (!/\.\w+$/.test(src)) {
        return conn.next(() => conn.bundle(`${contentDir}/${src}`))
          .then(partial => {
