        ...
        if (isBundle) {
          return conn.bundle(_opts.entry || `${contentDir}/${_opts.src || src}`, _opts.locals)
            ...
            .then(partial => {
              ...
              conn.set_state('environment', $.env);

              if (typeof _opts.locals === 'object') {
                conn.merge_state(_opts.locals);
              }
            ...
            .then(partial => {
              if (typeof conn.view === 'function') {
                conn.view(partial.render, { as: 'yield' });
              } else {
                conn.set_state('yield', conn.resp_body);
                conn.resp_body = partial.render(conn.state);
              }
            })
            .catch(e => {
