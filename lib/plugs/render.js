...
module.exports = function $render() {
  ...
  function _lookup(src) {
    for (let i = 0, c = args.length; i < c; i += 1) {
      const file = path.join(args[i], `${src}.js`);
  ...
  return ($, util) => {
    const _views = [];
    const _extras = $.get('viewsDir', []);

    // append app views
    args.unshift(path.join($.get('cwd', process.cwd()), $.get('appDir', 'app'), 'views'));

    // append extra views
    (!Array.isArray(_extras) ? [_extras] : _extras)
      .forEach(cwd => args.unshift(path.join($.get('cwd', process.cwd()), $.get('appDir', 'app'), cwd)));
