    ...
    $.extensions('Conn', {
      before_send() {
        ...
        if (this.handler && this.handler._controller && this.handler._controller.instance) {
          ...
          debug('#%s Reducing blocks from <%s.%s> views', this.pid, this.handler.controller, this.handler.action);
      ...
      methods: {
        view(src, locals) {
          ...
          debug('#%s Appending view <%s> as %s', this.pid, src, _target);
        ...
        render(src, locals) {
          ...
          debug('#%s Rendering view <%s>', this.pid, src);
