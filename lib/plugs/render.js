  ...
  function _render(tpl, util, _locals) {
    ...
    try {
      ...
      return util.reduce(tpl.data)
        .then(locals => {
          try {
            return _call(_fn || require(_cachedPaths[_id]), util.extend(locals, _locals));
          } catch (e) {
            e.debug = {
  ...
  return ($, util) => {
    ...
    function _view(conn, blocks) {
      let _layout = ((typeof conn.layout === 'undefined' ? conn.resp_locals.layout : conn.layout) !== false)
        ? (conn.resp_locals.layout || conn.layout || 'layouts/default')
        : '';
      ...
      if (conn.handler && conn.handler._controller && conn.handler._controller.instance) {
        ...
        _layout = _layout !== false && typeof _layout === 'string' ? `layouts/${_layout || 'default'}` : _layout;
      }
      ...
      return _render({ src: _layout, data: blocks }, util)
        .then(result => {
          /* istanbul ignore else */
          if (result && result.indexOf('<html') > -1 && !reDoctype.test(result)) {
    ...
    $.extensions('Conn', {
      ...
      methods: {
        ...
        render(src, locals) {
          ...
          const block = typeof src === 'string' ? src.split('/').pop() : null;
