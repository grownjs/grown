  ...
  for (let i = 0; i < _ids.length; i += 1) {
    ...
    try {
      ...
      if (typeof _id === 'function') {
        ...
        if (!cached[_id]) {
          cached[_id] = _tpl.find(_id, folders, i === _ids.length - 1);
        }
      }
      ...
      if (!view.data.render) {
        view.data.render = (tpl, state) =>
          _render({ src: tpl, data: state || {} }, cached, folders);
      }
      ...
      if (_id && cached[_id]) {
        /* istanbul ignore else */
        if (cached[_id].indexOf('.js') === -1) {
          return fs.createReadStream(cached[_id]);
        }

        return _tpl.run(require(cached[_id]), view.data);
      }
    } catch (e) {
...
function _view($, data, cached, folders, controllers) {
  ...
  const _controller = (controllers && $.handler.controller)
    ? util.get(controllers, $.handler.controller)
    : null;
...
function _partial(view, state) {
  /* istanbul ignore else */
  if (typeof view !== 'object') {
    view = { src: view, data: state };
  }

  state = view.data || state || {};

  const block = view.as || state.as || (typeof view.src === 'string'
    ? view.src.split('/').pop()
    : 'yield');

  delete state.as;

  return util.props(state)
        ...
        return {
          before_send() {
            ...
            const _controller = (_extensions.controllers && this.handler.controller)
              ? util.get(_extensions.controllers, this.handler.controller)
              : null;
            ...
            return Promise.resolve()
              // pre-resolve all locals
              .then(() => util.props(this.state).then(x => { this.state = x; }))
              .then(() => Promise.all(_views))
              // partial views
              .then(results =>
                results.reduce((prev, cur) => prev.then(() => {
                  util.extend(cur.data, _blocks, this.state);
              ...
              .then(() =>
                _view(this,
                  util.extend({}, _blocks, this.state),
          ...
          methods: {
            view(src, state) {
              _views.push(_partial(src, state || {}));
            ...
            render(src, state) {
              return _partial(src, util.extend({}, state, this.state))
