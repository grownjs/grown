...
function _view($, data, cached, folders, controllers) {
  ...
  return Promise.resolve()
    .then(() => _render({ src: _layout, data }, cached, folders, false, $.env))
    .then(result => {
      /* istanbul ignore else */
...
function _partial(view, data) {
  ...
  return { src: view.src, data, block, fallthrough: view.fallthrough };
}
      ...
      before_send() {
        ...
        return Promise.resolve()
          .then(() => {
            ...
            // FIXME: this has a fatal flaw!!
            // since controller loading is lazy there is no matching for keypaths... until
            // the request load any related controller... solution? just load all controllers
            // on the startup, even it would help to validate them, it would fix this issue...
          ...
          .then(results =>
            results.reduce((prev, cur) => prev.then(() => {
              ...
              return Promise.resolve()
                .then(() => _render(cur, _cached, _folders, false, $.env))
                .then(result => {
                  /* istanbul ignore else */
