...
function _render(view, cached, folders, fallthrough) {
    ...
    try {
      ...
      if (!view.data.render) {
        view.data.render = (tpl, state) =>
          _render({ src: tpl, data: state || {} }, cached, folders, fallthrough);
      }
...
function _partial(view, state) {
  ...
  return util.props(state)
    .then(data => ({ src: view.src, data, block, fallthrough: view.fallthrough }));
}
  ...
  return $ => {
    ...
    $.extensions('Conn', {
      ...
      before_send() {
        ...
        debug('Look-up into %s', _folders.join(', '));

        return Promise.resolve()
          .then(() => Promise.all(this.slots.templates))
          // partial views
          .then(results =>
          ...
          // catch-all
          .catch(e => errorHandler(e, this, null, $.options))
          // final view
          .then(() =>
            _view(this,
              util.extend({
