...
function _dispatch(err, conn) {
  ...
  if (!conn.halted) {
    debug('#%s Wait. Trying to finalize the connection', conn.pid);
...
function _closeAll(done) {
  ...
  FARMS.forEach(farm => {
    _tasks.push(() => {
      farm.close();
      _util.clearModules(farm.get('cwd', process.cwd()));
    });
  });
...
function Grown(opts) {
  ...
  return $({
    init() {
      ...
      // shortcuts
      (opts.use || []).forEach(cb => cb(this, util));
      (opts.mount || []).forEach((name, cb) => _mount.call($scope, name, cb));

      return {
        props: {
          env: () => $scope._opts.env || process.node.NODE_ENV || 'development',
        },
        methods: {
          // plugins
          use(cb) {
...
$new.readOnlyProperty(Grown, 'test', cb =>
  function $beforeTest(done) {
    cb.teardown(() => {
      ...
      this.fetch = Grown.plugs.testing(farm);

      return farm.emit('start').then(() =>
        farm.listen('test://').then(() => done()))
      .catch(done);
    });
  }, true);
