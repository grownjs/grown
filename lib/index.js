...
function Grown(opts) {
  ...
  return $({
    init() {
      ...
      function _get(key, defvalue) {
        let value;

        try {
          value = util.get(util.extend({}, $scope._opts), key, defvalue);
        } catch (e) {
        ...
        props: {
          ...
          util: () => ({
            ...
            contents: (file, fromPath) =>
              fs.readFileSync(path.join(_get('cwd'), _get(fromPath || 'paths.publicDir'), file)),
          }),
          logger: () => logger,
        },
        methods: {
          ...
          stop(cb) {
            return this.emit('close').then(() =>
              ...
              .then(() => _util.clearModules(_get('cwd')))
              .then(() => cb && cb());
          },
