...
function _finalizer(conn, callback) {
  return Promise.resolve()
    .then(() => callback())
    .then(() => debug('#%s Done.', conn.pid))
    .catch(e => {
      debug('#%s %s', conn.pid, e.stack);

      conn.end(e.statusCode || 500, e.message);
    });
}
...
function _dispatch(err, conn) {
  ...
  if (!(conn.halted || conn.has_views)) {
    return _finalizer(conn, () => conn.raise(501));
  }

  return _finalizer(conn, () => conn.end());
}
  ...
  return $({
    init() {
      ...
      const _utils = util.extend({}, util, {
        fs: {
          read: file =>
            fs.readFileSync(path.join(_opts.cwd, file)).toString(),
          write: (file, content) =>
            fs.outputFileSync(path.join(_opts.cwd, file), content),
          contents: (file, fromPath) =>
            fs.readFileSync(path.join(_opts.cwd, _getConfig(fromPath || 'paths.publicDir'), file)),
        },
      });
