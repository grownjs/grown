    ...
    init() {
      ...
      const _utils = util.extend({}, util, {
        collect: (filter, fromPath) =>
          glob.sync(filter, { cwd: _getConfig(fromPath || 'paths.config') })
            .map(srcDir => {
              const name = path.basename(path.dirname(srcDir));
              const config = path.join(_opts.cwd, _getConfig(fromPath || 'paths.config'), srcDir);
        ...
        contents: (file, fromPath) =>
          fs.readFileSync(path.join(_opts.cwd, _getConfig(fromPath || 'paths.public'), file)),
      });
