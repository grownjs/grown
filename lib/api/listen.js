...
module.exports = (context, container) => {
  context.listen = (location, options, callback) => {
    ...
    app.port = +(app.location.protocol === 'http:' && (options.cert && options.key)
      ? (app.location.port || 443)
      : (app.location.port || 80));

    app.location.host = app.location.host.split(':')[1]
      ? app.location.host
      : `${app.location.host}:${app.port}`;
