...
function safe(str) {
  ...
  if (str === null || typeof str === 'undefined') {
    return `<span class="empty">${str}</span>`;
  }
...
<div is-error>
  <div class="call">
    ...
    <p class="vars">${Object.keys($.context.resp_locals)
      .filter(x => typeof $.context[x] === 'undefined')
      .map(x => `<var>${x} <small>${typeof $.context.resp_locals[x]}</small></var>`)
      .join(' ')}</p>
  </div>
    ...
    <dl>
      ${Object.keys($.context).map(key =>
        (Array.isArray($.context[key])
          ...
          ? `<dt>${key}</dt><dd>${safe($.context[key])}</dd>\n`
          : '')).join('')}
    </dl>
  </details>
