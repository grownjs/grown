...
function _run(task, state, options) {
  ...
  /* istanbul ignore else */
  if (task.filter && !task.filter(state, options)) {
    debug('#%s Skip. Filter out', state.pid);
    return;
  }
  ...
  return function $pipeline(state, options) {
    ...
    function next(end) {
      ...
      if (!cb) {
        ...
        /* istanbul ignore else */
        if (state.halted) {
          debug('#%s OK. Pipeline halted on <%s>', state.pid, cb.name);
        ...
        try {
          ...
          e.summary = `Pipeline ${cb.name} was errored.`;
