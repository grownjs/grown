  ...
  return function $pipeline(state, options) {
    ...
    function next(end) {
      ...
      if (!cb) {
        ...
        try {
          ...
          debug('#%s Pipeline <%s> was errored on <%s>: %s', state.pid, label, cb.name, e.message);
          end(e);
          return;
        }
    ...
    return new Promise((resolve, reject) => {
      next(err => {
        ...
        /* istanbul ignore else */
        if (err) {
          err.pipeline = _stack.slice().reverse();

          debug('#%s Wait. Pipeline <%s> %s was errored', state.pid, label, _stack.join('.'));
        }
