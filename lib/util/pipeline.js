  ...
  return function $pipeline(state, options) {
    ...
    function next(end) {
      ...
      if (!cb) {
        ...
        /* istanbul ignore else */
        if (state.halted) {
          debug('#%s Wait. Pipeline <%s> was halted before <%s>', state.pid, label, cb.name);
        ...
        state.next = _resume => {
          ...
          debug('#%s Next. Trying to resolve the pending pipeline', state.pid);
        ...
        try {
          ...
          debug('#%s Pipeline <%s> was errored on <%s>: %s', state.pid, label, cb.type, e.message);
          end(e);
          return;
        }
    ...
    debug('#%s Initializing <%s> pipeline with %s task%s',
      state.pid,
      label,
      pipeline.length,
      pipeline.length === 1 ? '' : 's');
    ...
    return new Promise((resolve, reject) => {
      next(err => {
        ...
        if (err) {
          ...
          debug('#%s Wait. Pipeline <%s> was errored: %s', state.pid, label, err.message);
        } else {
          debug('#%s Done. Pipeline <%s> was ended', state.pid, label);
        }

        /* istanbul ignore else */
        if (!err && _callback) {
          debug('#%s Executing the <%s> pipeline callback', state.pid, label);

          try {
            retval = _callback(err, state, options);
          } catch (_err) {
            debug('#%s Wait. Pipeline <%s> callback was errored: %s', state.pid, label, _err.message);
