...
module.exports = function $host($, req, res) {
  ...
  debug('Initializing connection for %s%s', hostname, req.url.split('?')[0]);
  ...
  conn.res.on('finish', () => {
    debug('OK. Connection already finished');
  });

  conn.res.on('error', () => {
    debug('Wait. An error was ocurred');
  });

  this.dispatch(conn, this._opts)
    .catch((err) => {
      debug('Wait. The connection was errored: %s', err.message);
    ...
    .then(() => {
      debug('Done. The connection was successfully executed');
    });
};
