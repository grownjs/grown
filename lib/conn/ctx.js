  ...
  init($, server, req, res) {
    ...
    return {
      props: {
        ...
        // response body
        is_finished: () => _state.resp_body !== null,
      ...
      methods: {
        ...
        json(value) {
          ...
          if (_state.is !== 'blank' || res.finished) {
            return Promise.reject(new Error(`Connection is ${_state.is}`));
          }
          ...
          return Promise.resolve()
            ...
            .catch(e => {
              try {
                // FIXME: errorHandler(e, this, !this.is_xhr);

                if (!this.res.finished) {
                  res.writeHead(this.status_code, this.resp_headers);
...
module.exports = ($, _server, req, res) => {
  ...
  conn.before_send(() => {
    ...
    return Promise.all(tasks.map(cb => cb.call(conn)));
  });
