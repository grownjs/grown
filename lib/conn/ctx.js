...
module.exports = (container, server, req, res) => {
  const $ = util.extend({}, container.extensions);
  ...
  // $.extensions can be mutated directly at any IoC level
  // when copied, values are passed as read-only using
  // Object.defineProperty()

  // this prevents complex objects (e.g. circular) from
  // being serialized, visibility is a matter of check for IoC:

  // methods() -- all are non-enumerable
  // props() -- getters are hidden, scalar values are enumerated

  // a) util.props(conn, { models: () => _models, })
  // b) util.props(conn, { locals: _locals })

  // on (a) the value is resolved as property but is not enumerated
  // on (b) the value is resolved as property, also is being enumerated
  ...
  util.methods($, {
    ...
    params: () => util.extend({}, $.path_params, $.query_params, $.body_params),
