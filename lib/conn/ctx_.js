    ...
    function fail(e) {
      try {
        if (!res.finished) {
          ...
          res.write(errorHandler(e, this, !this.is_xhr, $._ctx.options));
          res.end();
        } else {
      ...
      methods: {
        ...
        cached(key, ttl, cb) {
          ...
          if (typeof $._data[key] !== 'undefined'
            && (new Date() - $._data[key].now) < (ttl * 1000)) {
        ...
        json(value) {
          ...
          if (code instanceof Error) {
            _resp.body = errorHandler(code, this, !this.is_xhr, $._ctx.options);
          } else {
            // normalize output
...
module.exports = function $ctx(server, req, res) {
  // before_send filters
  // const tasks = [];

  // this._extensions.forEach(mixin => {
  //   /* istanbul ignore else */
  ...
  //       /* istanbul ignore else */
  //       if (!conn.halted) {
  //         debug('#%s Call before_send(%s)', conn.pid, mixin.identifier || '?');

  //         try {
  //           return mixin.before_send.call(conn);
  //         } catch (e) {
  //           e.summary = `Error in before_send(${mixin.identifier || '?'})`;
  //           throw e;
  //         }
  //       }
  //     });
  ...
  // conn.before_send(() => {
  //   debug('#%s Call before_send(set_context_locals)', conn.pid);
