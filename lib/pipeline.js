  ...
  return function $pipeline(state, options) {
    ...
    function next(end) {
      ...
      if (!cb) {
        ...
        /* istanbul ignore else */
        if (state.halted) {
          debug('Wait. Pipeline was halted before %s task', cb.name);
        ...
        try {
          ...
          debug('The %s pipeline was errored on %s task: %s', label, cb.type, e.message);
          end(e);
          return;
        }
    ...
    return new Promise((resolve, reject) => {
      next((err) => {
        ...
        if (err) {
          ...
          debug('Wait. The %s pipeline was errored: %s', label, err.message);
        } else {
        ...
        if (!err && _callback) {
          ...
          try {
            ...
            debug('Wait. The %s pipeline callback was errored: %s', label, _err.message);
