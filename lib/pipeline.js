      ...
      return new Promise(function (resolve, reject) {
        function next(err, value) {
          ...
          if (!result.done) {
            ...
            /* istanbul ignore else */
            if (_scalar || typeof result.value === 'undefined' || result.value === null) {
            ...
            if (Array.isArray(result.value)) {
              Promise.all(result.value).then(function (_values) {
                next(undefined, _values);
              }).catch(next);
              return;
            }
            ...
            if (typeof result.value.then === 'function' && typeof result.value.catch === 'function') {
              result.value.then(function (_value) {
