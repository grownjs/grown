...
module.exports = function $error(err, conn, layout, options) {
  ...
  if (typeof conn.end === 'function') {
    conn.put_resp_content_type(_type);
  ...
  return (layout && _accept === 'html')
    ? _errorLayout(_msg, _err.code, conn.resp_charset || options('charset', 'utf8'))
    : _msg;
};
