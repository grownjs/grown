...
function _is(type, value, suffix) {
  const subject = Object.prototype.toString.call(value).match(/ (\w+)/)[1].toLowerCase();
  ...
  const msg = (types.length && last)
    ? `${types.join(', ')} or ${last}`
    : last;

  throw new Error(`Expecting a ${msg}${suffix ? ` ${suffix}` : ''}, given '${value}' (${subject})`);
}
